...
_ASSET_DIR = os.path.join(os.path.dirname(__file__), 'gait10dof18musc_assets')

SUITE = containers.TaggedTasks()


def make_model(floor_size):
  """Sets floor size"""
  #xml_string = common.read_model('gait10dof18musc.xml')
  xml_string = common.read_model(os.path.join(os.path.dirname(__file__), 'gait10dof18musc_rigid.xml'))
...
def get_model_and_assets(floor_size=10):
  ...
  return make_model(floor_size), assets
...
class Physics(mujoco.Physics):
  ...
  def extremities(self):
    ...
    for side in ('_l', '_r'):
      for limb in ('talus',):
        torso_to_limb = self.named.data.xpos[limb + side] - torso_pos
